...
?>
<?php
$pdo = "SELECT * FROM album WHERE 1";
$Pdotement = $bdd->prepare($pdo);
$result = $Pdotement->execute();
	...
	<div id="album">
		<ul class="card-list">
			<?php foreach ($result as $results) { ?>
				<li class="card">
					<a class="card-image" href="player.php?id=<?=$results['id']?>" aria-current="page" target="_blank">
						<img src="./musique/<?= $results["image"] ?>" />
					</a>
					<a class="card-description" href="player.php?id=<?=$results['id']?>" aria-current="page" target="_blank">
